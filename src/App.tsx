...
import { DualReportInput } from './components/DualReportInput';
import { ProfileMetaForm } from './components/ProfileMetaForm';
...
function App() {
  ...
  const canGenerate = reportA.trim().length > 100 && reportB.trim().length > 100 && meta.handle.trim();
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Step 1: Extract data
      setLoadingStep(1);
      const response = await extractChannelData({
        reportA,
        reportB,
        meta,
      });
              ...
              <div className="space-y-8 animate-fade-in">
                ...
                <section>
                  ...
                  <DualReportInput
                    reportA={reportA}
                    reportB={reportB}
                    onChangeA={setReportA}
                    onChangeB={setReportB}
                  />
                </section>

                {/* Profile Meta */}
                <section className="max-w-md">
                  <ProfileMetaForm meta={meta} onChange={setMeta} />
                </section>
                ...
                <section className="pt-4">
                  ...
                  {!canGenerate && (
                    <p className="text-sm text-gray-500 mt-2">
                      * 두 보고서 모두 100자 이상 입력하고 핸들을 입력해주세요
                    </p>
                  )}
                </section>
              </div>
